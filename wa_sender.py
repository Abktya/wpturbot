...
def send_watch_list(to: str, tours: list[dict], target_gbp: float, dest_key: str) -> bool:
    ...
    Kullanıcı tıklayınca list_reply gelir → _watch_cache'ten tur bilgisi alınır.
    max 10 tur (WA limiti)
    """
    ...
    rows = []
    for t in tours[:10]:
        if not t.get('url') or not t.get('price_gbp'):
            continue                        # URL'siz turları atla

        diff    = t['price_gbp'] - target_gbp
        sign    = '+' if diff >= 0 else ''
        price_s = f"£{t['price_gbp']:.0f}({sign}£{int(abs(diff))})"
        ...
        if t.get('nights'):
            desc += f" | {t['nights']} gece" if desc else f"{t['nights']} gece"
        desc = (desc or t.get('source', ''))[:72]

        # Kısa hash ID (8 char) — tam URL yerine
